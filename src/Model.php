...
abstract class Model implements ArrayAccess, JsonSerializable
{
    ...
    {
        return method_exists($this, 'get'.StringHelpers::studly($key).'Attribute');
    }
    ...
    {
        return $this->{'get'.StringHelpers::studly($key).'Attribute'}($value);
    }
    ...
    protected function castAttribute($key, $value)
    {
        ...
        switch ($this->getCastType($key)) {
            ...
            // Collection is not supported for now
            // case 'collection':
    ...
    public function setAttribute($key, $value)
    {
        ...
        if ($this->hasSetMutator($key)) {
            $method = 'set'.StringHelpers::studly($key).'Attribute';
    ...
    {
        return method_exists($this, 'set'.StringHelpers::studly($key).'Attribute');
    }
    ...
    /**
     * Get associative array of attributes except the attributes from list
     *
     * @param array<string> $except
     * @return array
     */
    private function getAttributesExcept(array $except = [])
    {
        $result = [];
    ...
    public function replicate(array $except = null)
    {
        ...
        $attributes = $this->getAttributesExcept((array) $except);
        ...
        if (preg_match_all('/(?<=^|;)get([^;]+?)Attribute(;|$)/', implode(';', get_class_methods($class)), $matches)) {
            foreach ($matches[1] as $match) {
                if (static::$snakeAttributes) {
                    $match = StringHelpers::snake($match);
                }
